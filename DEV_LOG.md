...
> **勘误（2026-07-15）**：本节"14/14 接口全部 OK"是 **akshare + 旧 mootdx + pandas 2.x** 旧实现快照。5 天后 v0.2.5（2026-05-17）移除 akshare 重写、v0.2.7 替换失效接口、v0.2.15 升级 mootdx 0.11.x 等变更改变了数据层，但 `test_data_quality.py` 自 2026-05-13 后再未复跑，导致 5 个 bug 潜伏 2 个月（v0.2.19/v0.2.20 修复）。详见 `issues/008-week5-quality-gate-stale-snapshot.md`。
...
- ✅ **v0.2.27 分析性能优化** - 增加节点/模型/工具/持久化耗时台账和 P50/P95 汇总；同次分析缓存相同成功请求且失败不缓存；七分析师使用独立消息通道并行汇合；新增保留技术/新闻/基本面的快速模式，质量门控、东财限流和数据缺失约束保持不变。
- ✅ **v0.2.27 服务器 E2E** - 600879 完整分析 8分01秒（7/7 报告、34 工具调用、4 次复用、主力资金缺失后门控为中），快速分析 6分33秒（3/3 核心报告、17 工具调用全成功、范围硬提示生效）；真实运行发现并修复 timed_node 将 Trader 的可选 `name` 误当 `config` 的兼容问题。
